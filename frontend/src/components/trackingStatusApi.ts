...
const BROWSER_RELAY_HINT = IS_LOCAL_BROWSER_RUNTIME ? LOCAL_BROWSER_RELAY_HINT : HOSTED_EDGE_SERVICE_HINT;
...
export const lookupTrackingInCarrierPortal = async (
  ...
  if (!supportsEdgeLookup(carrier)) {
    return {
      ...
      carrier,
      trackingNumber,
      error: `${EDGE_LOOKUP_DISABLED_HINT} ${IS_LOCAL_BROWSER_RUNTIME ? LOCAL_BROWSER_RELAY_HINT : HOSTED_EDGE_SERVICE_HINT}`.trim(),
    };
  }
  ...
  if (!supportsLivePortalLookup(carrier)) {
    return {
      ...
      carrier,
      trackingNumber,
      error: `La consulta viva de ${carrier.toUpperCase()} no está disponible en esta configuración. ${EDGE_LOOKUP_DISABLED_HINT}`,
    };
  }
