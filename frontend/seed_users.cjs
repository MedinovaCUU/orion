...
async function run() {
  ...
  for (const emp of rows) {
    ...
    if (authError) {
        ...
        // Usually "User already exists", we still want to upsert the profile
        const { data: userExists } = await supabase.auth.admin.listUsers();
        const existingUser = userExists.users.find(u => u.email === emp.email);
        if (existingUser) {
            const role = emp.es_admin === true || emp.es_admin === 'true' ? 'admin' : 'tecnico';
            
            let formattedPhone = emp.cellphone ? emp.cellphone.toString().trim() : null;
            if (formattedPhone && !formattedPhone.startsWith('+')) {
                formattedPhone = '+52' + formattedPhone;
            }
            if (formattedPhone) {
                await supabase.auth.admin.updateUserById(existingUser.id, { phone: formattedPhone });
            }
            ...
            await supabase.from('profiles').upsert({
              ...
              territorio: emp.territory || null,
              recibe_tickets: emp.gets_tickets === true || emp.gets_tickets === 'true'
            });
            console.log(`Updated profile for existing user ${emp.email}`);
        }
        continue;
    }
    ...
    if (formattedPhone) {
        await supabase.auth.admin.updateUserById(userId, { phone: formattedPhone });
    }
